...
public:
	...
	{
		declare_parameter("firmware_path", std::string("/home/ansoon/NovintFalcon/nvent_firmware.bin"));
		...
		device_.setFalconGrip<libnifalcon::FalconGripFourButton>();  

		if (!device_.open(static_cast<unsigned int>(dev_idx))) {
			throw std::runtime_error("Cannot open Falcon (device " + std::to_string(dev_idx) +") – is it plugged in?");
		}
		...
		RCLCPP_INFO(get_logger(),
			...
			"Pull the grip out then push in to home (LED turns green).", dev_idx, rate_hz);
		RCLCPP_INFO(get_logger(),
...
~FalconNode()
{
	...
	device_.runIOLoop(libnifalcon::FalconDevice::FALCON_LOOP_FIRMWARE | libnifalcon::FalconDevice::FALCON_LOOP_KINEMATIC);
	device_.close();
...
void loop()
{
	...
	fw->setLEDStatus(homed ? libnifalcon::FalconFirmware::GREEN_LED : libnifalcon::FalconFirmware::RED_LED);
